...
`sander` es un CLI que gestiona sandboxes de agentes de IA: crea un sandbox (box vía agentbox con el proyecto teleportado y su config del harness sincronizada), ejecuta prompts dentro de forma headless, y permite entrar y controlarlo al estilo docker (attach/exec). Inyecta el GitHub token y las variables de entorno en el sandbox para que el agente empuje su rama sin más fricción. Está construido sobre seams estrechos (provider, harness) para que cambiar o mejorar Sander no toque el resto.
...
25. Como usuario, quiero que el agente, en su configuración inicial dentro del sandbox, genere el script de arranque y elija el nombre de la rama, para que el arranque y la rama reflejen la decisión del agente sobre mi proyecto.
...
- **Seam `tracker` (reducido)**: no hay orquestación de issues ni PRs. Sander solo resuelve e inyecta el GitHub token en el sandbox; la rama git la crea el agente dentro, que elige su nombre. El token se resuelve con precedencia: flag del comando > config global > config workspace (futura) > `gh auth token` con confirmación del usuario.
...
- **Leer issues, crear ramas/PRs desde Sander, o el flujo inverso "en base al issue X"**. El agente usa el token inyectado para su flujo GitHub; Sander no orquesta.
